/*  Copyright (C) 2011-2015  P.D. Buchan (pdbuchan@yahoo.com)
    Copyright (C) 2018 Kirils Solovjovs (research@kirils.org)
...
main (int argc, char **argv)
{
  ...
  if( argc != 4 ) {
	printf("Usage %s <interface> <source_ip> <signature>\n\n",argv[0]);
	exit(1);
  }
  ...
  udphdr.source = htons (6790);

  // Destination port number (16 bits): pick a number
  udphdr.dest = htons (6790);
